...
pagemod.PageMod({
  ...
  onAttach: function(worker) {
    ...
    worker.port.on("returnMicroData", forwardMicroDataToPanel);
  }
});
function forwardMicroDataToPanel(data) {
  if(mdViewerPanel) {
    mdViewerPanel.port.emit("updateMicroData", data);
  }
}
...
function createMdViewerPanel() {
  var mdPanel = panel.Panel({
    ...
    contentScriptFile: require("sdk/self").data.url("mdViewerPanel.js"),
    position: showMdButton,
    onHide: handlePanelHide
  });
  mdPanel.port.on("extractMicroData", function() {
